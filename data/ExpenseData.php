...
FROM
	...
	WHERE public.accounts.account_type = 'EXPENSE' AND public.budget_amounts.period_num = date_part('month', CURRENT_DATE)) t1
FULL JOIN
	(SELECT public.Accounts.name AS account, round((SUM(public.Splits.value_num)/AVG(public.accounts.commodity_scu)),2) AS value
	...
	GROUP BY public.accounts.name) t2
ON
